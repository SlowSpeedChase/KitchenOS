...
- [x] Note whether any phrase works without saying "KitchenOS" (observation for the phrase-token question; record in BRANCH-STATUS notes).
  ...
  **Superseded 2026-08-13 ~10:31 CDT — the answer is stronger than "keep the token":** Type-to-Siri with the byte-exact registered phrase "Ask KitchenOS what's on my meal plan this week" (pasted to the device pasteboard, so ASR was out of the loop) ALSO got the generic fallback with zero server traffic. Registration is proven (shortcuts visible in the Shortcuts app; a tap-run of AskKitchenOS answered from real data at 09:57, `GET /api/meal-plan/2026-W33` → 200). Conclusion: conversational Siri on this iOS 27 beta does not consult App Shortcut phrase templates at all — an OS-level routing gap, not an app defect. Voice-adjacent entry until a beta fixes it: run the shortcut from the Shortcuts app / Home Screen. Retest on every beta bump.
