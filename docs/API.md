...
| `/api/recipes` 🔒 | GET | Recipe metadata index for the meal-planner sidebar. `?ingredient=<term>` filters to recipes whose ingredient list contains the case-insensitive substring — backs the Siri "recipes with X" intent. `?include_ingredients=1` additionally returns `ingredient_items` (each row's full ingredient list) — no new route, reuses this same gated 🔒 endpoint; backs the Spotlight ingredient-keyword donation (`KitchenOSClient.allRecipes(includeIngredients:)` → `RecipeIndexer.reindexAll` folds them into `attributeSet.keywords`). Cached 5 min. |
