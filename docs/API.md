...
| `/generate-shopping-list` | POST | Generate a shopping list markdown file from a meal plan. Body `{week, use_pantry?}`. **Credits current inventory but never decrements it.** A line is credited only when the normalized ingredient identity and usable units agree; broad related-food matches and inventory `ct` rows with unknown package contents stay as unchecked buy items and are explained under **Check pantry**. Confirmed full/partial credits appear under **Already have**. Both note sections use plain bullets, so only `## Items` checkboxes reach Reminders. Expired rows are ignored, water/ice are omitted, and `use_pantry: false` returns raw demand. The response includes `inventory_credited_count` and `inventory_review_count`; manually-added items are preserved using an encoded generated-item snapshot, with semantic cleanup for legacy notes whose old generator changed quantities or names. `/api/shopping-list/preview` → `/confirm` remains the only path that spends stock. |
...
| `/api/shopping-list/preview` | POST | **Pantry-aware shopping list, step 1.** Body `{week, use_pantry?}` → per-line records split into `from_pantry` / `to_buy` against current usable inventory. Each line includes `status` (`buy`, `credited`, `review`, or `excluded`) and `matched_inventory`; `review` never reduces `to_buy`. |
