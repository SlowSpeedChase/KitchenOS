...
from batch_extract import resolve_reminder_url
from main import instagram_parser, is_youtube_url, route_url
...
# is_youtube_url/route_url moved to main.py so extract_recipe.py's CLI could
# share them; batch_extract had been the only owner of the routing rule.

def test_instagram_reel_routes_to_reel_pipeline():
    assert route_url(REEL) == "instagram"
    assert not is_youtube_url(REEL)
    assert instagram_parser(REEL)


def test_youtube_takes_precedence_over_instagram():
    assert route_url(YT) == "youtube"
    assert is_youtube_url(YT)


def test_web_url_falls_through_to_scraper():
    assert route_url(WEB) == "web"
