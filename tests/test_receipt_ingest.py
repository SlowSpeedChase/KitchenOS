...
def test_purchase_insert_failure_rolls_back_and_retry_ingests_once(
    tmp_vault, tmp_db, alias_tmp, monkeypatch
):
    real_insert_purchases = idb._insert_purchases
    monkeypatch.setattr(
        idb,
        "_insert_purchases",
        lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("boom")),
    ...
    with pytest.raises(RuntimeError, match="boom"):
        ri.ingest_parsed(
            dict(PARSED_OK), source="photo_receipt", source_id="photo-purchases"
        )
    ...
    monkeypatch.setattr(idb, "_insert_purchases", real_insert_purchases)
    first_retry = ri.ingest_parsed(
        dict(PARSED_OK), source="photo_receipt", source_id="photo-purchases"
    )
    replay = ri.ingest_parsed(
        dict(PARSED_OK), source="photo_receipt", source_id="photo-purchases"
    )
    assert first_retry["status"] == "ingested"
    assert replay["status"] == "skipped"

    conn = idb.connect()
    try:
        assert conn.execute("SELECT COUNT(*) FROM trips").fetchone()[0] == 1
        assert conn.execute("SELECT COUNT(*) FROM purchases").fetchone()[0] == 4
