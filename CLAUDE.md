...
- **`data/kitchenos.db` is the single source of truth** for inventory/purchases/trips. `Inventory.md`, `Price Tracker.md`, `Use It Up.md`, and similar files are **generated, read-only views** — they carry a do-not-edit banner, are rewritten on every relevant change, and hand edits are silently overwritten.
