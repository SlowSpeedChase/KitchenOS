...
> ## ⛔ KitchenOS captions-only policy (overrides everything below)
> This is a **per-project, captions-only** vendored copy. It exists to read **on-screen**
> cooking details (measurements, steps, ingredients shown but not spoken) that the recipe
> pipeline's transcript misses — it is an agent-assist for recipe debugging, **not** a
> replacement for `extract_recipe.py`'s automated transcript stage.
> - **ALWAYS pass `--no-whisper`** on every `watch.py` invocation. No audio ever leaves the machine.
> - **NEVER** set, write, or prompt for `GROQ_API_KEY` / `OPENAI_API_KEY`, and **never** create
>   `~/.config/watch/.env` with a key. Skip Step 0's "encourage a Whisper key" guidance entirely.
> - If a video has **no captions**, return **frames-only** and say so plainly — do not fall back to Whisper.
> - `yt-dlp` + `ffmpeg` are already installed (pipx/brew); if Step 0 reports `needs_install`, stop and
>   tell the user rather than auto-installing.
> - Rationale: KitchenOS/.env carries an `OPENAI_API_KEY` for its *own* pipeline; `--no-whisper`
>   guarantees this skill can never use it to egress a caption-less video's audio.
